...
#  baseDir <- '/home/ntellini/t2tydna/T2TyDNA/tlo'
#  ind="BG50-SRE"
...
file.create("listemptychrsextremities.txt")
...
all_tel_forstats$chr <- sapply(strsplit(all_tel_forstats$chr_coord ,"-"),"[[",1)
all_tel_forstats$placement <- paste0(sapply(strsplit(all_tel_forstats$chr_coord ,"-"),"[[",2),"-",sapply(strsplit(all_tel_forstats$chr_coord ,"-"),"[[",3))
all_tel_forstats[,"side"] <- "R"
all_tel_forstats[grep("1-30000",all_tel_forstats$placement),"side"] <- "L"
...
summary_plot[summary_plot$Var2 == "L","xval"] <- min(all_tel$diff) - 500
summary_plot[summary_plot$Var2 == "R","xval"] <- max(all_tel$diff) + 500
