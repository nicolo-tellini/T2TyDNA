...
chrs=$(cat $inds".idxstat.30kblenmin.txt" | cut -f1)
...
do
echo $i 
echo -e "$i\t1\t30000" >> anchors.bed
end=$(grep -w $i $inds".idxstat.30kblenmin.txt" | cut -f2)
chr_internal_end=$(grep -w $i $inds".idxstat.30kblenmin.txt" | cut -f2 | awk '{print $1 - 30000}')
